...
class TestResetAll:
    #Test 1
    def test_OK(self):
        login = subprocess.Popen(["python", "login.py", "--username", "andreane82", "--passw", "e00f8e21a864de304a6c"]).wait()
        upload = subprocess.Popen(["python", "questionnaire_upd.py", "--source", file_path],stdout=subprocess.PIPE, stderr=subprocess.PIPE).wait()
